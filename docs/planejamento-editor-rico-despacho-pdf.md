...
**Contexto:** permitir que secretarias redijam despachos ricos dentro do GED, salvar versões, exportar PDF para o acervo (Paperless-ngx) e assinar digitalmente (fluxos simples e A1 já existentes), reduzindo dependência de Word externo + upload manual.

**Stack atual de referência:** NestJS (`apps/backend`), Next.js 15 (`apps/frontend`), Paperless via `PaperlessService.uploadDocument`, `ProcessDocument` (SHA, `paperlessDocumentId`), despacho hoje em `ProcessMovement.dispatchText` (texto plano).
...
|-----------|------|---------|
| **TipTap ou Lexical + JSON/HTML + PDF no servidor** | Boa integração Next + Nest; sem licença extra; fluxo “salvar → PDF → Paperless” controlável. | PDF fiel ao layout pode exigir motor tipo Chromium (Puppeteer/Playwright) ou biblioteca de layout. |
...
3. Ao **finalizar**: criar registro em **`ProcessDocument`** apontando para o **PDF** no Paperless (reuso do fluxo de anexos).
...
2. `POST .../despacho-editor/finalizar`:
   - renderizar HTML → gerar PDF → `paperless.uploadDocument` → gravar `ProcessDocument` (`title`, `mimeType`, `sha256`, `paperlessDocumentId`).
...
4. Pós-finalizar: atualizar lista de documentos; link para visualizar no Paperless se existir endpoint de download.
...
- [ ] PDF no Paperless com nome e metadados esperados.
...
2. PDF + Paperless + lista de documentos.
3. Assinatura no PDF.
...
- Garantir `PAPERLESS_API_URL` e conectividade Paperless em ambiente de teste.
